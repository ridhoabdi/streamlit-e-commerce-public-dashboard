...
def create_sum_order_items_df(df):
    sum_order_items_df = (
        df.groupby("product_category_name_english")["product_id"].count().reset_index()
    )
...
def create_bycity_df(df):
    ...
    most_common_city = bycity_df.loc[
        bycity_df["customer_count"].idxmax(), "customer_city"
    ]
...
def create_rfm_df(df):
    ...
    rfm_df["max_purchase_timestamp"] = pd.to_datetime(rfm_df["max_purchase_timestamp"])
    rfm_df["max_purchase_date"] = pd.to_datetime(
        rfm_df["max_purchase_timestamp"].dt.date
    )
    recent_date = df["order_purchase_timestamp"].dt.date.max()
    rfm_df["recency"] = (
        pd.to_datetime(recent_date) - rfm_df["max_purchase_date"]
    ).dt.days
...
# Load cleaned data
all_df = pd.read_csv(
    "https://raw.githubusercontent.com/ridhoabdi/streamlit-e-commerce-public-dashboard/main/merge-dataset/all_data.csv"
)
datetime_columns = [
    "order_approved_at",
...
with st.sidebar:
    ...
    st.image(
        "https://raw.githubusercontent.com/ridhoabdi/streamlit-e-commerce-public-dashboard/main/assets/logo-e-commerce.png",
        width=200,
    )
...
sum_order_items_df = (
    all_df.groupby("product_category_name_english")["product_id"].count().reset_index()
)
...
sns.barplot(
    ...
    data=bystate_df,
    palette=[
        "#72BCD4" if state == most_common_state else "#D3D3D3"
...
sns.barplot(
    ...
    y=bycity_df.values,
    palette=[
        "#72BCD4" if city == most_common_city else "#D3D3D3" for city in bycity_df.index
    ],
)
...
# Footer
st.caption("Copyright © Muhammad Ridho Abdillah 2023")
